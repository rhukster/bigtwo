// Big Two game engine - shared logic for client and server
import type { Card, Rank, Suit, PlayTypeResult, PlayType } from './types';
...
export function findStarterIndex(hands: Card[][]): number {
...
export function getPlayType(cards: Card[]): PlayTypeResult | null {
...
export function canBeat(newCards: Card[], currentPlay: Card[] | null, currentPlayType: PlayTypeResult | null): boolean {
  if (!currentPlay || !currentPlayType) return true;

  const newType = getPlayType(newCards);
  if (!newType) return false;
  if (newType.type !== currentPlayType.type) return false;

  return newType.str > currentPlayType.str;
}

// Get display name for a play type
export function getPlayTypeName(type: PlayType): string {
...
export function includesThreeOfDiamonds(cards: Card[]): boolean {
...
export function hasUnusedTwos(hand: Card[]): boolean {
  return hand.some(c => c.rank === '2');
}

// Check if hand contains unused four-of-a-kinds
export function hasUnusedQuads(hand: Card[]): boolean {
...
export function hasUnusedStraightFlush(hand: Card[]): boolean {
...
export function calculatePoints(
  ...
  hasUnusedTwos: boolean,
  hasUnusedQuads: boolean,
  hasUnusedStraightFlush: boolean,
  winnerEndedWithSpecial: boolean
): number {
  ...
  let multiplier = 1;
  if (hasUnusedTwos) multiplier *= 2;
  if (hasUnusedQuads) multiplier *= 2;
  if (hasUnusedStraightFlush) multiplier *= 2;
  if (winnerEndedWithSpecial) multiplier *= 2;
